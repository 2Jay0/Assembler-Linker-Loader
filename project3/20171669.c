...
int main()
{
    ...
    /*-----------------------------------------------*/
    int progaddr=0;
    int load_key=0; //estab에 넣을 때의 key값 값 하나당 1씩 증가함
    int load_flag=0; //loader 수행 시 형식 출력할 때 한번씩만 나와야하므로 이걸로 check
    int total_length=0; //load 되는 프로그램들의 길이의 합
    int run_total_length=0;
    int cur_addr=0; //값이 바뀌기 전의 함수 시작값 저장(ex proga)
    int bp_num=0;
    int run_progaddr=0; //run에 사용 될 progaddr
    int run_locctr=0; //run에 사용될 locctr
    int run_flag=0;
    int run_temp_progaddr=0;
    ...
    while (1) {
        ...
        else if (strcmp(check_str, "loader") == 0)
        {
            if(first_cnt==4)
            {
                ptr = strtok(temp_str, "\t ");
                ...
                if(cnt==4)
                {
                    ...
                    for(i=1;i<cnt;i++)
                    { 
                        strcpy(order,store[i]);
                        char* result = strstr(order,".obj");
                        if(result!=NULL)
                        {
                            if(load_flag==(cnt-2))
                                check_pass1=load_pass1(order,&progaddr,&load_key,-1,&total_length);
                            else
                                check_pass1=load_pass1(order,&progaddr,&load_key,load_flag++,&total_length);
                    ...
                    for(i=1;i<cnt;i++)
                    {
                        strcpy(order,store[i]);
                        char* result = strstr(order,".obj");
                        if(result!=NULL&&check_pass1==1)
                        {
                            check_pass2=load_pass2(order,&cur_addr);
                        }
                        else
                            printf("Wrong instruction!\n");
                    }
            ...
            else if(first_cnt==2)
            {
                ...
                if(cnt==2)
                {
                    ...
                    { 
                        ...
                        if(result!=NULL)
                        {
                            check_pass1=load_pass1(order,&progaddr,&load_key,-2,&total_length);
                        }
                        else
                            check_pass1=-3; 
                    }
        ...
        else if (strcmp(check_str, "progaddr") == 0)
        {
             if(first_cnt==2)
             {
                ptr = strtok(temp_str, "\t ");
                ...
                { 
                    ...
                    for (i = 0; i < num; i++)
                    {
                        ...
                        ex--;
                        if (store[1][i] >= 48 && store[1][i] <= 57)
                            hex_addr_num += (int)n_exp * ((int)store[1][i] - 48);

                        else if (store[1][i] >= 65 && store[1][i] <= 70)
                            hex_addr_num += (int)n_exp * ((int)store[1][i] - 55);

                        else if (store[1][i] >= 97 && store[1][i] <= 102)
                            hex_addr_num += (int)n_exp * ((int)store[1][i] - 87);
                        ...
                        progaddr=hex_addr_num; 
                        run_progaddr=hex_addr_num;
                        run_temp_progaddr=hex_addr_num;
                    }
                    if(hex_addr_num!=-1)
                    {
                        strcpy(perfect_str,store[0]);
        ...
        else if(strncmp(check_str, "bp", 2) == 0)
        {
            if(first_cnt==2)
             {
                ptr = strtok(temp_str, "\t ");
                ...
                else
                {
                    ...
                    for (i = 0; i < num; i++)
                    {
                        ...
                        ex--;
                        if (store[1][i] >= 48 && store[1][i] <= 57)
                            bp_num += (int)n_exp * ((int)store[1][i] - 48);
                        else if (store[1][i] >= 65 && store[1][i] <= 70)
                            bp_num += (int)n_exp * ((int)store[1][i] - 55);
                        else if (store[1][i] >= 97 && store[1][i] <= 102)
                            bp_num += (int)n_exp * ((int)store[1][i] - 87);
                        else
                        {
                            printf("Wrong instruction!\n");
                            break;
                    ...
                    mem_bp[bp_num]=1; //bp설정되어있으면 1로 설정
        ...
        else if(strcmp(check_str,"run")==0)
        {
            if(first_cnt==1)
            {
                run_locctr=run(&run_progaddr,run_total_length,run_locctr,run_flag++);//progaddr 부터 시작
                if(run_locctr==-1)
                {
                    run_locctr=0;
                    run_progaddr=run_temp_progaddr;
                }
